...
import { Sparkles, CalendarCheck, MapPin, Megaphone } from 'lucide-react';
import { cn } from '@/lib/utils';

const leftItems = [
  "We need more booked appointments without hiring multiple vendors",
  "We need stronger local SEO and better visibility on Google",
  "We need better ad creative and landing pages",
  "We need blog content and social posts that support growth",
  "We need reporting that ties activity to appointments"
];

const useCases = [
  {
    title: "Booking Recovery",
    description: "Pages, forms, calls-to-action, scheduling, mobile UX",
    icon: CalendarCheck,
    ...
    metrics: ["Pages", "Forms", "Mobile UX"],
    metric: "Booking Rate"
  },
  {
    title: "Trust + Visibility",
    description: "Local SEO, Google Business Profile, reviews, authority content",
    icon: MapPin,
    ...
    metrics: ["Local SEO", "GBP", "Reviews"],
    metric: "Visibility Score"
  },
  {
    title: "Creative + Campaign Lift",
    description: "Ad design, video editing, multi-channel optimization, reporting",
    icon: Megaphone,
    ...
    metrics: ["Ad Design", "Video", "Reporting"],
    metric: "Campaign Performance"
  }
];

export default function ClinicUseCases() {
  const [activeIndex, setActiveIndex] = useState(null);
  const [hoveredLeftItem, setHoveredLeftItem] = useState(null);

  const handleItemHover = (i) => {
    setHoveredLeftItem(i);
    // Map left items (5) to right cards (3) proportionally
    setActiveIndex(Math.min(i, useCases.length - 1));
  };
    ...
    <section className="py-24 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <motion.div
          ...
          <h2 className="text-3xl md:text-4xl font-bold"><span className="text-[#1a1a1a]">Common reasons clinics come to us</span></h2>
        </motion.div>
